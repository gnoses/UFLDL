...
%     load stlTrainSubset.mat % loads numTrainImages, trainImages, trainLabels
%     load stlTestSubset.mat  % loads numTestImages,  testImages,  testLabels
    load ('stlTrainSubset.mat' ,  'trainLabels');
    load ('stlTestSubset.mat'  ,  'testLabels');
...
%     numClasses = 5;
    ...
    nnInputData = permute(pooledFeaturesTrain, [1 3 4 2]);
    nnInputData = reshape(nnInputData, numel(pooledFeaturesTrain) / numTrainImages,...
        numTrainImages);
    nnOutputData = trainLabels;

    
    
    inputSize = numel(pooledFeaturesTrain) / numTrainImages;
    theta = initializeParameters(hiddenSize, inputSize, numClasses);
    [opttheta, cost] = minFunc( @(p) nnCost(p, inputSize, ...
        hiddenSize, numClasses, nnInputData,nnOutputData,lambda), ...                                   
        theta, options);
end
%%======================================================================
...
nnInputData = reshape(nnInputData, numel(pooledFeaturesTrain) / numTrainImages, numTrainImages);
nnOutputData = trainLabels;
[cost] = nnTest(opttheta, inputSize, hiddenSize, numClasses, nnInputData,nnOutputData, lambda);

% fprintf('Train Data Accuracy: %2.3f%%\n', acc * 100);



nnInputData = permute(pooledFeaturesTest, [1 3 4 2]);
nnInputData = reshape(nnInputData, numel(pooledFeaturesTest) / numTestImages, numTestImages);
nnOutputData = testLabels;
