...
if DEBUG
    ...
    [cost grad] = cnnCost(db_theta,db_images,db_labels,numClasses,...
                                db_filterDim,db_numFilters,db_poolDim,0);
    

    % Check gradients
    numGrad = computeNumericalGradient( @(x) cnnCost(x,db_images,...
                                db_labels,numClasses,db_filterDim,...
                                db_numFilters,db_poolDim,0), db_theta);
 
    % Use this to visually compare the gradients side by side
%     disp([numGrad grad]);
    
    imageDim = size(images,1);
    [Wc, Wd, bc, bd] = cnnParamsToStack(grad,imageDim,db_filterDim,db_numFilters,...
                        db_poolDim,numClasses);
                    
    [Wc2, Wd2, bc2, bd2] = cnnParamsToStack(numGrad,imageDim,db_filterDim,db_numFilters,...
                        db_poolDim,numClasses);
...
if (1)
    ...
    opttheta = minFuncSGD(@(x,y,z) cnnCost(x,y,z,numClasses,filterDim,...
                          numFilters,poolDim,0),theta,images,labels,options);
    t = toc();
    str = sprintf('%.1f min (%d sec)', t / 60, t);
    disp(str);
    save ('opttheta.mat', 'opttheta');
else
    load ('opttheta.mat', 'opttheta');
end
%%======================================================================
%% STEP 4: Test
